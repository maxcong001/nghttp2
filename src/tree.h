...
public:
  ...
  int addHandler(HandleFunc handler,
...
template <typename requestTypename, typename responseTypename> class RadixTree {
...
public:
  ...
  ~RadixTree() { delete this->root; }
  int insert(const std::string &path, HandleFunc handler,
             const std::vector<std::string_view> &methods) {
    // to do remove '/'
    auto root = this->root;
    ...
    while (i < n) {
      if (!root->indices.empty() &&
          (root->indices[0] == kAsterisk || path[i] == kAsterisk ||
           (path[i] != kColon && root->indices[0] == kColon) ||
           (path[i] == kColon && root->indices[0] != kColon) ||
           (path[i] == kColon && root->indices[0] == kColon &&
            path.substr(i + 1, find(path, kSlash, i) - i - 1) !=
      ...
      if (!child) {
        ...
        if (p == n) {
          ...
          root = root->insertChild(
              path[i], new RadixTreeNode<requestTypename, responseTypename>(
                           path.substr(i, p - i)));

          if (p < n) {
            root = root->insertChild(
                kAsterisk, new RadixTreeNode<requestTypename, responseTypename>(
                               path.substr(p + 1)));
            ++paramCount;
          }
        ...
        root = root->insertChild(
            kColon, new RadixTreeNode<requestTypename, responseTypename>(
                        path.substr(p + 1, i - p - 1)));
        ++paramCount;
        ...
        if (path[i] == kColon) {
          ...
          if (j < m) {
            auto child = new RadixTreeNode<requestTypename, responseTypename>(
                root->path.substr(j));
...
private:
  ...
  RadixTreeNode<requestTypename, responseTypename> *root;
};
